...
[Source](../src/schemars/lib.rs.html#1-190)

Generate JSON Schema documents from Rust code

## Basic Usage

For more detailed information, see the full [API documentation on docs.rs](https://docs.rs/schemars/latest) and the [detailed usage documentation website](https://graham.cool/schemars).
...
`#[serde(...)]` attributes can be overridden using `#[schemars(...)]` attributes, which behave identically (e.g. `#[schemars(rename_all = "camelCase")]`). You may find this useful if you want to change the generated schema without affecting Serde’s behaviour, or if you’re just not using Serde.
...
- External libraries that are supported via optional dependencies (see [Feature Flags](#feature-flags)) *may* be removed in a minor version change, particularly if a newer semver-incompatible version has been released for a long time.
...
- `raw_value` – implements `JsonSchema` for `serde_json::value::RawValue` (enables the serde\_json `raw_value` feature)
...
- `rust_decimal1` – [rust\_decimal](https://crates.io/crates/rust_decimal) (^1.0)
- `semver1` – [semver](https://crates.io/crates/semver) (^1.0.9)
- `smallvec1` – [smallvec](https://crates.io/crates/smallvec) (^1.0)
- `smol_str02` – [smol\_str](https://crates.io/crates/smol_str) (^0.2.1)
- `smol_str03` – [smol\_str](https://crates.io/crates/smol_str) (^0.3)
...
- [consts](consts/index.html) – Constants associated with JSON Schema generation.
- [generate](generate/index.html) – JSON Schema generator and settings.
- [transform](transform/index.html) – Contains the `Transform` trait, used to modify a constructed schema and optionally its subschemas. This trait is automatically implemented for functions of the form `fn(&mut Schema) -> ()`.

## Macros

- [json\_schema](macro.json_schema.html) – Construct a `Schema` from a JSON literal. This can either be a JSON object, or a boolean (`true` or `false`).
- [schema\_for](macro.schema_for.html) – Generates a `Schema` for the given type using default settings. The default settings currently conform to [JSON Schema 2020-12](https://json-schema.org/specification-links#2020-12), but this is liable to change in a future version of Schemars if support for other JSON Schema versions is added.
- [schema\_for\_value](macro.schema_for_value.html) – Generates a `Schema` for the given example value using default settings. The default settings currently conform to [JSON Schema 2020-12](https://json-schema.org/specification-links#2020-12), but this is liable to change in a future version of Schemars if support for other JSON Schema versions is added.

## Structs

- [Schema](struct.Schema.html) – A JSON Schema.
- [SchemaGenerator](struct.SchemaGenerator.html) – The main type used to generate JSON Schemas.

## Traits

- [JsonSchema](trait.JsonSchema.html) – A type which can be described as a JSON Schema document.

## Derive Macros

- [JsonSchema](derive.JsonSchema.html) – Derive macro for `JsonSchema` trait.
- [JsonSchema\_repr](derive.JsonSchema_repr.html) – Derive macro for `JsonSchema_repr` trait.
