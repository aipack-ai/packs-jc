...
// Create a Schema for an object with property `foo`
let mut schema: Schema = json_schema!({"type": "object", "properties": {"foo": true}});
...
The `visit` module and `Visitor` trait have been replaced with `transform` and `Transform` respectively. Accordingly, these items have been renamed:
